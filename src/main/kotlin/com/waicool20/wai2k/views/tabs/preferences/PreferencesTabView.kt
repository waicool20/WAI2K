    ...
    fun initializeTree() {
        ...
        PreferencesViewMappings.list.forEach { node ->
            ...
            if (node.parent == null) {
                ...
                PreferencesViewMappings.list.find { it.masterView == node.parent }?.children?.add(
                    node
                )
            }
        }
        preferencesTreeView.focusModel.focusedItemProperty()
            .addListener("PreferenceFocused") { newVal ->
